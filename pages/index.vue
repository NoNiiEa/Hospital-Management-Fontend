<template>
    <div class="min-h-screen bg-blue-50 p-6 px-20">
        ...
        <!-- Doctor Info -->
        <div
            class="bg-blue-900 text-white p-6 mt-4 rounded-lg flex items-center"
        >
            <div
                class="w-16 h-16 bg-yellow-400 rounded-full flex items-center justify-center text-xl font-bold"
            >
                👨‍⚕️
            </div>
            ...
            <div v-if="doctor_REF" class="ml-4">
                <h2 class="text-xl font-semibold">
                    คุณหมอ {{ doctor_REF.name }}
                </h2>
                <p class="text-sm opacity-75">
                    ผู้เชี่ยวชาญด้าน {{ doctor_REF.specialization }}
                </p>
            </div>
            <div v-else class="ml-4 text-sm opacity-75">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
            ...
            <div class="bg-white p-6 rounded-lg shadow-md">
                ...
                <ul class="mt-3">
                    <li
                        v-for="(scheduleItem, index) in doctor_REF?.schedule ||
                        []"
                        :key="index"
                        class="p-2 border-b"
                    >
                        ...
                        >:
                        <span
                            v-for="(time, tIndex) in scheduleItem.timeslot"
                            :key="tIndex"
                        >
                            {{ time
                            }}<span
                                v-if="
                                    tIndex !== scheduleItem.timeslot.length - 1
                                "
                                >,
                            </span>
                        </span>
            ...
            <div class="bg-white p-6 rounded-lg shadow-md">
                ...
                <ul class="mt-3">
                    <li
                        v-for="(patientItem, index) in doctor_REF?.patients ||
                        []"
                        :key="index"
                        class="p-2 border-b"
                    >
                        ...
                        <router-link
                            :to="`/patient/${patientItem.patient_id}`"
                            class="text-blue-900 hover:underline"
                        >
                            <strong>{{
                                patientNames[patientItem.patient_id] ||
                                "กำลังโหลด..."
                            }}</strong>
                        </router-link>
                        <br />
...
const fetchPatientData = async (patientId) => {
    try {
        const response = await fetch(
            `http://127.0.0.1:8000/patients/get/${patientId}`,
        );
        if (response.ok) {
